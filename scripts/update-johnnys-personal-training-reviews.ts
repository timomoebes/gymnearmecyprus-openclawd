/**
 * Update Johnny's Personal Training Paphos - Review Count
 * 
 * Updates review count to 27 everywhere (database field and description text).
 * 
 * Usage:
 *   npx tsx scripts/update-johnnys-personal-training-reviews.ts
 */

import { supabaseAdmin } from '../lib/supabase/admin-client';

async function updateJohnnysPersonalTrainingReviews() {
  console.log('🔄 Updating Johnny\'s Personal Training Paphos review count...\n');
  console.log('   - Review count: 27\n');
  ...
  try {
    ...
    const slugs = ['johnnys-personal-training', 'johnnys-personal-training-paphos'];
    ...
    if (!gym) {
      const { data: gymsByName } = await supabaseAdmin
        .from('gyms')
        .select('id, name, slug, review_count, description')
        .ilike('name', '%Johnny%Personal Training%')
        .limit(1);
    ...
    if (!gym) {
      ...
      console.error('   Tried name search: Johnny Personal Training');
      return;
    }
    ...
    // Step 2: Update description to replace any review count mentions with 27
    console.log('📝 Step 2: Preparing updates...');
    
    // Update description to replace any review count mentions with 27
    let updatedDescription = gym.description || '';
    updatedDescription = updatedDescription
      .replace(/\b\d+\s+reviews?\b/gi, '27 reviews')
      .replace(/from\s+\d+\s+reviews?\b/gi, 'from 27 reviews')
      .replace(/\(\d+\s+reviews?\)/gi, '(27 reviews)')
      .replace(/with\s+\d+\s+reviews?\b/gi, 'with 27 reviews')
      .replace(/\b\d+\s+rating[s]?\b/gi, '27 reviews')
      .replace(/rated\s+\d+/gi, 'rated 27');
    
    console.log('   New review_count: 27');
    ...
    console.log('');
    console.log('🎉 Successfully updated Johnny\'s Personal Training Paphos!');
    console.log('   ✓ Review count: 27');
...
updateJohnnysPersonalTrainingReviews();
