...
Processes raw Apify Google Maps scraper data for Paphos gyms.
...
RAW_DATA_PATH = PROJECT_ROOT / "data" / "raw" / "paphos_gyms_raw.csv"
CLEAN_DATA_DIR = PROJECT_ROOT / "data" / "clean"
CLEAN_DATA_DIR.mkdir(exist_ok=True, parents=True)
CLEAN_CSV_PATH = CLEAN_DATA_DIR / "paphos_gyms_clean.csv"
TEST_SAMPLE_PATH = CLEAN_DATA_DIR / "paphos_gyms_test_sample.csv"
DOCS_DIR = PROJECT_ROOT / "docs"
DOCS_DIR.mkdir(exist_ok=True, parents=True)

# Constants
PAPHOS_CITY_ID = "7978d742-eeea-4c7a-b37f-7dceacd4284b"
PAPHOS_CENTER_LAT = 34.7756
PAPHOS_CENTER_LNG = 32.4244

# Paphos neighborhoods (to include in city filter)
PAPHOS_NEIGHBORHOODS = [
    "Paphos", "Pafos", "Kato Paphos", "Kato Pafos",
    "Konia", "Coral Bay", "Peyia", "Tala", "Tremithousa",
    "Mesogi", "Yeroskipou", "Emba", "Chloraka", "Geroskipou"
]
...
def is_paphos_area(city):
    """Check if city is in Paphos or its neighborhoods"""
    if pd.isna(city):
        return False
    city_str = str(city).strip()
    return any(neighborhood.lower() in city_str.lower() for neighborhood in PAPHOS_NEIGHBORHOODS)

def clean_data():
    """Main cleaning function"""
    print("=" * 80)
    print("GYM DATA CLEANING: Paphos Gyms")
    print("=" * 80)
    print()
    ...
    # For Paphos, we'll be more lenient - only remove if both street and city are missing
    ...
    # Ensure Paphos-based (city should be Paphos or its neighborhoods)
    before_city = len(df)
    df = df[df['city'].apply(is_paphos_area)].copy()
    city_filtered = before_city - len(df)
    print(f"  Removed {city_filtered} rows not in Paphos area")
    ...
    clean_df['city_id'] = PAPHOS_CITY_ID
    ...
    for idx in test_sample.index:
        ...
        if orig_idx:
            ...
            city = orig_row['city'] if pd.notna(orig_row['city']) else 'Paphos'
            if street:
                ...
                print(f"    [WARN] No street address for: {test_sample.loc[idx, 'name']}, using fallback")
            time.sleep(1)  # Rate limiting
