...
Processes raw Apify Google Maps scraper data for Nicosia gyms.
...
RAW_DATA_PATH = PROJECT_ROOT / "data" / "raw" / "nicosia_gyms_raw.csv"
CLEAN_DATA_DIR = PROJECT_ROOT / "data" / "clean"
CLEAN_DATA_DIR.mkdir(exist_ok=True, parents=True)
CLEAN_CSV_PATH = CLEAN_DATA_DIR / "nicosia_gyms_clean.csv"
TEST_SAMPLE_PATH = CLEAN_DATA_DIR / "nicosia_gyms_test_sample.csv"
DOCS_DIR = PROJECT_ROOT / "docs"
DOCS_DIR.mkdir(exist_ok=True, parents=True)

# Constants
NICOSIA_CITY_ID = "fa1518c7-1daf-4e05-a27b-d6296176ef2e"
NICOSIA_CENTER_LAT = 35.1856
NICOSIA_CENTER_LNG = 33.3823

# Nicosia neighborhoods (to include in city filter)
NICOSIA_NEIGHBORHOODS = [
    "Nicosia", "Strovolos", "Egkomi", "Aglantzia", "Agios Dometios",
    "Ortakoy", "Engomi", "Lakatamia", "Latsia", "Makedonitissa"
]
...
def is_nicosia_area(city):
    """Check if city is in Nicosia or its neighborhoods"""
    if pd.isna(city):
        return False
    city_str = str(city).strip()
    return any(neighborhood.lower() in city_str.lower() for neighborhood in NICOSIA_NEIGHBORHOODS)

def clean_data():
    """Main cleaning function"""
    print("=" * 80)
    print("GYM DATA CLEANING: Nicosia Gyms")
    print("=" * 80)
    print()
    ...
    # Ensure Nicosia-based (city should be Nicosia or its neighborhoods)
    before_city = len(df)
    df = df[df['city'].apply(is_nicosia_area)].copy()
    city_filtered = before_city - len(df)
    print(f"  Removed {city_filtered} rows not in Nicosia area")
    ...
    print("  [INFO] Using fallback coordinates for now (Nicosia center)")
    print("  [INFO] Test sample will be geocoded individually")
    
    # Use fallback coordinates for all (will geocode test sample separately)
    df['latitude'] = NICOSIA_CENTER_LAT
    df['longitude'] = NICOSIA_CENTER_LNG
    ...
    clean_df['city_id'] = NICOSIA_CITY_ID
