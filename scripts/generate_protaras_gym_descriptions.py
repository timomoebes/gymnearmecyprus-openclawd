"""
Generate SEO-optimized descriptions for all Protaras gyms
...
def generate_description(gym_row):
    ...
    # Extract city from address (usually "Protaras, Cyprus" or neighborhood)
    city = 'Protaras'
    if 'Protaras' in address:
        city = 'Protaras'
    elif 'Paralimni' in address or 'Kapparis' in address or 'Pernera' in address:
        city = 'Protaras'  # These are Protaras areas
    ...
    if address and address != 'null' and address != '':
        ...
        if len(address_parts) > 0:
            ...
            if location and not location.startswith('XX'):  # Skip Plus Code addresses
...
def main():
    ...
    CLEAN_CSV_PATH = PROJECT_ROOT / "data" / "clean" / "protaras_gyms_clean.csv"
    SQL_OUTPUT_PATH = PROJECT_ROOT / "scripts" / "update_protaras_descriptions.sql"
    
    print("=" * 80)
    print("GENERATING SEO DESCRIPTIONS FOR PROTARAS GYMS")
    print("=" * 80)
    print()
    ...
    with open(SQL_OUTPUT_PATH, 'w', encoding='utf-8') as f:
        f.write("-- Update SEO descriptions for Protaras gyms\n")
