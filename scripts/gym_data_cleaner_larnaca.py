...
Processes raw Apify Google Maps scraper data for Larnaca gyms.
...
RAW_DATA_PATH = PROJECT_ROOT / "data" / "raw" / "larnaca_gyms_raw.csv"
CLEAN_DATA_DIR = PROJECT_ROOT / "data" / "clean"
CLEAN_DATA_DIR.mkdir(exist_ok=True, parents=True)
CLEAN_CSV_PATH = CLEAN_DATA_DIR / "larnaca_gyms_clean.csv"
TEST_SAMPLE_PATH = CLEAN_DATA_DIR / "larnaca_gyms_test_sample.csv"
DOCS_DIR = PROJECT_ROOT / "docs"
DOCS_DIR.mkdir(exist_ok=True, parents=True)

# Constants
LARNACA_CITY_ID = "c2208ba4-aea9-4e22-b256-09177179763f"
LARNACA_CENTER_LAT = 34.9167
LARNACA_CENTER_LNG = 33.6333

# Larnaca neighborhoods (to include in city filter)
LARNACA_NEIGHBORHOODS = [
    "Larnaca", "Larnaka", "Larnaka City", "Larnaka Town",
    "Makenzy", "Mackenzie", "Pyla", "Dromolaxia", "Aradippou",
    "Livadia", "Oroklini", "Kiti", "Perivolia", "Tersefanou"
]
...
FILTER_OUT_CATEGORIES = [
    ...
    "Sportswear store",  # Added for Larnaca
...
def is_larnaca_area(city):
    """Check if city is in Larnaca or its neighborhoods"""
    if pd.isna(city):
        return False
    city_str = str(city).strip()
    return any(neighborhood.lower() in city_str.lower() for neighborhood in LARNACA_NEIGHBORHOODS)

def clean_data():
    """Main cleaning function"""
    print("=" * 80)
    print("GYM DATA CLEANING: Larnaca Gyms")
    print("=" * 80)
    print()
    ...
    # Ensure Larnaca-based (city should be Larnaca or its neighborhoods)
    before_city = len(df)
    df = df[df['city'].apply(is_larnaca_area)].copy()
    city_filtered = before_city - len(df)
    print(f"  Removed {city_filtered} rows not in Larnaca area")
    ...
    print("  [INFO] Using fallback coordinates for now (Larnaca center)")
    print("  [INFO] Test sample will be geocoded individually")
    
    # Use fallback coordinates for all (will geocode test sample separately)
    df['latitude'] = LARNACA_CENTER_LAT
    df['longitude'] = LARNACA_CENTER_LNG
    ...
    clean_df['city_id'] = LARNACA_CITY_ID
