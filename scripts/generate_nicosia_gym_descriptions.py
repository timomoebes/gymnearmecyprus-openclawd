"""
Generate SEO-optimized descriptions for all Nicosia gyms
...
def generate_description(gym_row):
    ...
    # Extract city from address (usually "Nicosia, Cyprus" or neighborhood)
    city = 'Nicosia'
    if 'Nicosia' in address:
        city = 'Nicosia'
    elif 'Strovolos' in address:
        city = 'Nicosia'  # Strovolos is part of Nicosia
    elif 'Egkomi' in address or 'Aglantzia' in address:
        city = 'Nicosia'  # These are Nicosia neighborhoods
...
def main():
    ...
    CLEAN_CSV_PATH = PROJECT_ROOT / "data" / "clean" / "nicosia_gyms_clean.csv"
    SQL_OUTPUT_PATH = PROJECT_ROOT / "scripts" / "update_nicosia_descriptions.sql"
    
    print("=" * 80)
    print("GENERATING SEO DESCRIPTIONS FOR NICOSIA GYMS")
    print("=" * 80)
    print()
    ...
    with open(SQL_OUTPUT_PATH, 'w', encoding='utf-8') as f:
        f.write("-- Update SEO descriptions for Nicosia gyms\n")
