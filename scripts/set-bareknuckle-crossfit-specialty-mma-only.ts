/**
 * BareKnuckle CrossFit Larnaca: remove Fitness/Gym specialty, keep only Martial Arts & MMA.
 * Run: npx tsx scripts/set-bareknuckle-crossfit-specialty-mma-only.ts
 */
...
async function main() {
  const gymSlug = 'bareknuckle-crossfit-larnaca';
  ...
  if (!gym) {
    console.error('❌ Gym not found:', gymSlug);
    process.exit(0);
  }
  ...
  console.log(`✅ ${gym.name}: specialty set to "${spec.name}" only (Fitness/Gym removed).`);
}
