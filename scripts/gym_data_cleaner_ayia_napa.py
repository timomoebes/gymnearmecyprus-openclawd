...
Processes raw Apify Google Maps scraper data for Ayia Napa gyms.
...
RAW_DATA_PATH = PROJECT_ROOT / "data" / "raw" / "ayia_napa_gyms_raw.csv"
CLEAN_DATA_DIR = PROJECT_ROOT / "data" / "clean"
CLEAN_DATA_DIR.mkdir(exist_ok=True, parents=True)
CLEAN_CSV_PATH = CLEAN_DATA_DIR / "ayia_napa_gyms_clean.csv"
TEST_SAMPLE_PATH = CLEAN_DATA_DIR / "ayia_napa_gyms_test_sample.csv"
DOCS_DIR = PROJECT_ROOT / "docs"
DOCS_DIR.mkdir(exist_ok=True, parents=True)

# Constants
AYIA_NAPA_CITY_ID = "209d0e59-baa3-4a6d-88fa-1e908747bdb3"
AYIA_NAPA_CENTER_LAT = 34.9881
AYIA_NAPA_CENTER_LNG = 34.0125

# Ayia Napa neighborhoods (to include in city filter)
AYIA_NAPA_NEIGHBORHOODS = [
    "Ayia Napa", "Agia Napa", "Ayia Napa Town", "Agia Napa Town",
    "Nissi Beach", "Makronissos", "Cape Greco", "Protaras"  # Protaras is nearby
]
...
def is_ayia_napa_area(city):
    """Check if city is in Ayia Napa or its neighborhoods"""
    if pd.isna(city):
        return False
    city_str = str(city).strip()
    return any(neighborhood.lower() in city_str.lower() for neighborhood in AYIA_NAPA_NEIGHBORHOODS)

def clean_data():
    """Main cleaning function"""
    print("=" * 80)
    print("GYM DATA CLEANING: Ayia Napa Gyms")
    print("=" * 80)
    print()
    ...
    # For Ayia Napa, we'll be more lenient - only remove if both street and city are missing
    ...
    # Filter by rating (keep >= 3.0 or null) - More lenient for small dataset
    ...
    # Filter by reviews (keep >= 1 or null) - More lenient for small dataset
    before_reviews = len(df)
    df = df[(df['reviewsCount'].isna()) | (df['reviewsCount'] >= 1)].copy()
    reviews_filtered = before_reviews - len(df)
    print(f"  Removed {reviews_filtered} rows with < 1 reviews")
    
    # Ensure Ayia Napa-based (city should be Ayia Napa or its neighborhoods)
    before_city = len(df)
    df = df[df['city'].apply(is_ayia_napa_area)].copy()
    city_filtered = before_city - len(df)
    print(f"  Removed {city_filtered} rows not in Ayia Napa area")
    ...
    print("  [INFO] Using fallback coordinates for now (Ayia Napa center)")
    print("  [INFO] Test sample will be geocoded individually")
    
    # Use fallback coordinates for all (will geocode test sample separately)
    df['latitude'] = AYIA_NAPA_CENTER_LAT
    df['longitude'] = AYIA_NAPA_CENTER_LNG
    ...
    clean_df['city_id'] = AYIA_NAPA_CITY_ID
    ...
    for idx in test_sample.index:
        ...
        if orig_idx:
            ...
            city = orig_row['city'] if pd.notna(orig_row['city']) else 'Ayia Napa'
            if street and street != 'XXPG+VX7':  # Skip Plus Code addresses
