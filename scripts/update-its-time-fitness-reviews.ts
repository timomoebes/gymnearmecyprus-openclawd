/**
 * Update It's Time Fitness Center Larnaca - Review Count
 * 
 * Updates review count to 140 everywhere (database field and description text).
 * 
 * Usage:
 *   npx tsx scripts/update-its-time-fitness-reviews.ts
 */

import { supabaseAdmin } from '../lib/supabase/admin-client';

async function updateItsTimeFitnessReviews() {
  console.log('🔄 Updating It\'s Time Fitness Center Larnaca review count...\n');
  console.log('   - Review count: 140\n');
  ...
  try {
    ...
    const slugs = ['its-time-fitness-center-larnaca', 'its-time-fitness-center', 'its-time-fitness'];
    ...
    if (!gym) {
      ...
      console.error('   Tried name search: Time Fitness');
      return;
    }
    ...
    // Step 2: Update description to replace any review count mentions with 140
    console.log('📝 Step 2: Preparing updates...');
    
    // Update description to replace any review count mentions with 140
    let updatedDescription = gym.description || '';
    updatedDescription = updatedDescription
      .replace(/\b\d+\s+reviews?\b/gi, '140 reviews')
      .replace(/from\s+\d+\s+reviews?\b/gi, 'from 140 reviews')
      .replace(/\(\d+\s+reviews?\)/gi, '(140 reviews)')
      .replace(/with\s+\d+\s+reviews?\b/gi, 'with 140 reviews')
      .replace(/\b\d+\s+rating[s]?\b/gi, '140 reviews')
      .replace(/rated\s+\d+/gi, 'rated 140');
    
    console.log('   New review_count: 140');
    ...
    console.log('');
    console.log('🎉 Successfully updated It\'s Time Fitness Center Larnaca!');
    console.log('   ✓ Review count: 140');
